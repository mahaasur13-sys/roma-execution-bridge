...
class LogRequestMiddleware(BaseHTTPMiddleware):
    async def dispatch(self, request: Request, call_next):
        ...
        print(
            f"[ROMA] {request.method} {request.url.path} → {response.status_code} ({duration_ms:.1f}ms)"
        )
        return response
...
def auth_middleware(request: Request) -> dict:
    ...
    try:
        ...
        raise HTTPException(
            429, {"error": {"code": "QUOTA_EXCEEDED", "message": str(e)}}
        )
    except RateLimitExceeded as e:
        raise HTTPException(429, {"error": {"code": "RATE_LIMITED", "message": str(e)}})
    except Exception:
        raise HTTPException(
            401, {"error": {"code": "INVALID_KEY", "message": "Invalid API key"}}
        )
