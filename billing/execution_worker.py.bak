...
async def execute_and_bill(
    ...
    while waited < max_wait:
        try:
            ...
            if status in ("completed", "running"):
                # Если есть SSH host, попробуем запустить команду
                if status == "running" and backend_name == "vastai":
    ...
    try:
        # Кредитуем баланс (для теста), потом дебитуем стоимость
        ...
        _billing_ledger.debit(tenant_id, cost_usd, "USD",
        ...
        _db_adapter.record_usage_event(tenant_id, "gpu_execution", elapsed, cost_usd, job_id,
        ...
        _db_adapter.update_execution_job(job_id, status="completed",
        ...
        _db_adapter.update_execution_job(job_id, status="completed",
                                          completed_at=now_iso, cost_usd=cost_usd,
                                          error="Billing error: " + str(exc)[:500])
