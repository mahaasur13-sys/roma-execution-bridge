...
try:
    ...
    pass
def _dup(event_id):
    return _redis and _redis.exists(f"stripe:event:{event_id}") > 0
def _mark(event_id):
    if _redis:
        _redis.setex(f"stripe:event:{event_id}", 86400, str(time.time()))
def _verify(payload, sig, secret):
    if not sig or not secret: return True  # Skip in dev
    try:
...
class Response(BaseModel):
    received: bool; event_id: Optional[str] = None
    processed: bool = False; error: Optional[str] = None
@router.post("/stripe", response_model=Response)
async def stripe_webhook(request: Request, x_stripe_signature: Optional[str] = Header(None)):
    ...
    if _dup(eid): return Response(received=True, event_id=eid, processed=False)
    ok = False
    try:
        ...
        if etype == "invoice.paid":
            ...
            if share["revenue_share_cents"] > 0:
                ledger.record_revenue_share(tid, share["revenue_share_cents"],
