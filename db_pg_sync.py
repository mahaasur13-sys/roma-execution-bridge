...
def init_db(conn) -> None:
    ...
    """)
    cur.execute(
        "CREATE INDEX IF NOT EXISTS idx_processed_invoices_inv ON processed_invoices(invoice_id)"
    )
    cur.execute("""
...
def set_tenant_inactive(conn, tenant_id: str) -> None:
    cur = conn.cursor()
    cur.execute(
        "UPDATE tenants SET subscription_status='inactive', updated_at=now() WHERE id=%s",
        (tenant_id,),
    )
    conn.commit()
...
def record_webhook_event(
    conn, stripe_event_id: str, event_type: str, tenant_id: str, payload: str
) -> None:
    cur = conn.cursor()
    cur.execute(
...
def list_webhook_events(conn, limit: int = 20) -> list[dict]:
    cur = conn.cursor()
    cur.execute(
        "SELECT * FROM webhook_events ORDER BY created_at DESC LIMIT %s", (limit,)
    )
    cols = [desc[0] for desc in cur.description]
...
def mark_invoice_processed(
    conn, invoice_id: str, event_type: str = "", tenant_id: str = ""
) -> None:
    cur = conn.cursor()
    cur.execute(
...
def get_analytics_overview(conn, days: int = 30) -> dict:
    cur = conn.cursor()
    cur.execute(
        "SELECT count(*) FROM user_events WHERE created_at >= now() - interval '%s days'",
        (days,),
    )
    total = cur.fetchone()[0]
    cur.execute(
        "SELECT count(DISTINCT tenant_id) FROM user_events WHERE created_at >= now() - interval '%s days'",
        (days,),
    )
...
def get_analytics_events(
    ...
    cur.execute(
        f"SELECT * FROM user_events {where} ORDER BY created_at DESC LIMIT %s OFFSET %s",
        params + [limit, offset],
    )
    cols = [desc[0] for desc in cur.description]
...
def get_feedback(
    ...
    cur.execute(
        f"SELECT * FROM feedback {where} ORDER BY created_at DESC LIMIT %s OFFSET %s",
        params + [limit, offset],
    )
    cols = [desc[0] for desc in cur.description]
    rows = [dict(zip(cols, row)) for row in cur.fetchall()]
...
def get_tenant_workers(conn, tenant_id: str) -> list[dict]:
    cur = conn.cursor()
    cur.execute(
        "SELECT * FROM workers WHERE tenant_id = %s ORDER BY created_at DESC",
        (tenant_id,),
    )
    cols = [desc[0] for desc in cur.description]
    return [dict(zip(cols, row)) for row in cur.fetchall()]
...
def drain_worker(conn, worker_id: str) -> None:
    cur = conn.cursor()
    cur.execute(
        "UPDATE workers SET drained=true, updated_at=now() WHERE id=%s", (worker_id,)
    )
    conn.commit()


def register_worker(
    conn, worker_id: str, tenant_id: str, capabilities: dict = None
) -> None:
    import json
...
def update_worker_heartbeat(conn, worker_id: str) -> None:
    cur = conn.cursor()
    cur.execute(
        "UPDATE workers SET last_heartbeat=now(), updated_at=now() WHERE id=%s",
        (worker_id,),
    )
    conn.commit()
...
def release_worker(conn, worker_id: str) -> None:
    cur = conn.cursor()
    cur.execute(
        "UPDATE workers SET status='idle', updated_at=now() WHERE id=%s", (worker_id,)
    )
    conn.commit()
...
def create_job_idempotency(
    conn, tenant_id: str, idempotency_key: str, job_id: str
) -> bool:
...
def create_email_user(
    conn, email: str, password_hash: str, name: str, tenant_id: str, api_key: str
) -> dict:
...
def store_verification_token(
    conn, email: str, token_hash: str, expires_at: str
) -> None:
    cur = conn.cursor()
    cur.execute(
        "UPDATE users SET verification_token = %s, verification_token_expires_at = %s, "
...
def use_invite_code(conn, invite_code_id: int, user_id: str) -> None:
    ...
    cur.execute(
        "INSERT INTO invite_usage (invite_code_id, user_id) VALUES (%s,%s)",
