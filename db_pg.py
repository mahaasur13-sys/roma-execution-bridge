...
async def update_tenant_subscription(
    ...
    async with pool.acquire() as conn:
        ...
        if stripe_customer_id:
            parts.append(f"stripe_customer_id = ${idx}")
            params.append(stripe_customer_id)
            idx += 1
        if stripe_subscription_id:
            parts.append(f"stripe_subscription_id = ${idx}")
            params.append(stripe_subscription_id)
            idx += 1
        if subscription_status:
            parts.append(f"subscription_status = ${idx}")
            params.append(subscription_status)
            idx += 1
        if plan:
            parts.append(f"plan = ${idx}")
            params.append(plan)
            idx += 1
        if subscription_end_date:
            parts.append(f"subscription_end_date = ${idx}")
...
async def list_webhook_events(limit: int = 20) -> list[dict]:
    ...
    async with pool.acquire() as conn:
        rows = await conn.fetch(
            "SELECT * FROM webhook_events ORDER BY received_at DESC LIMIT $1", limit
        )
    return [dict(r) for r in rows]
...
async def mark_invoice_processed(
...
async def list_leads(status: str = "") -> list[dict]:
    ...
    async with pool.acquire() as conn:
        if status:
            rows = await conn.fetch(
                "SELECT * FROM leads WHERE status = $1 ORDER BY created_at DESC", status
            )
        else:
...
async def update_email_event(recipient_email: str, event_type: str) -> None:
    ...
    async with pool.acquire() as conn:
        if event_type == 'open':
            ...
            await conn.execute(
                "UPDATE email_logs SET status='bounced' WHERE recipient_email=$1",
                recipient_email,
            )
...
async def get_analytics_overview(days: int = 30) -> dict:
    ...
    async with pool.acquire() as conn:
        ...
        active_today = await conn.fetchval(
            "SELECT COUNT(DISTINCT tenant_id) FROM user_events WHERE created_at >= now() - interval '1 day'"
        )
        active_week = await conn.fetchval(
            "SELECT COUNT(DISTINCT tenant_id) FROM user_events WHERE created_at >= now() - interval '7 days'"
        )
        active_month = await conn.fetchval(
            "SELECT COUNT(DISTINCT tenant_id) FROM user_events WHERE created_at >= now() - interval '30 days'"
        )

        login_users = await conn.fetchval(
            "SELECT COUNT(DISTINCT tenant_id) FROM user_events WHERE event_type='login'"
        )
        submit_users = await conn.fetchval(
        ...
        total_submits = await conn.fetchval(
            "SELECT COUNT(*) FROM user_events WHERE event_type='job_submit'"
        )
        total_completes = await conn.fetchval(
            "SELECT COUNT(*) FROM user_events WHERE event_type='job_complete'"
        )
        submit_to_complete = (
            round(total_completes / total_submits, 2) if total_submits else 0
        )
        ...
        for r in rows:
            ...
            if r["event_type"] == "login":
                logins[idx] = r["cnt"]
            elif r["event_type"] == "job_submit":
                submits[idx] = r["cnt"]
            elif r["event_type"] == "job_complete":
                completes[idx] = r["cnt"]
...
async def get_feedback(
    ...
    if to_date:
        ...
        idx += 1
    if rating > 0:
        where.append(f"rating = ${idx}")
        params.append(rating)
        idx += 1
    wh = " AND ".join(where)
    async with pool.acquire() as conn:
        total = await conn.fetchval(
            f"SELECT COUNT(*) FROM feedback WHERE {wh}", *params
        )
        rows = await conn.fetch(
...
async def get_tenant_workers(tenant_id: str) -> list[dict]:
    ...
    async with pool.acquire() as conn:
        rows = await conn.fetch(
            "SELECT * FROM workers WHERE tenant_id = $1 ORDER BY created_at DESC",
            tenant_id,
        )
    return [dict(r) for r in rows]
