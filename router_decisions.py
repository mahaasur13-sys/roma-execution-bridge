...
async def create_decision(payload: dict, x_api_key: str = Header(None)):
    ...
    # Policy check
    pol = evaluate_policies(tenant_id, "decision.create", {
        "request_type": request_type, "estimated_cost": 0.01,
        "priority": pay.get("priority", 5),
    })
    if pol["result"] != "allowed":
        write_audit_event(tenant_id, "policy.denied", "decision", str(uuid.uuid4()),
    ...
    if idem_key:
        ...
        if prev:
            return {"decision_id": prev.get("id"), "result": prev.get("gate_result"),
                    "reason": prev.get("gate_reason"), "estimated_cost": prev.get("estimated_cost", 0),
                    "quota_remaining": prev.get("quota_remaining", 0), "job_id": None, "idempotent": True}
    ...
    db.insert_decision_record(did, rid, tenant_id, decision.result.value, decision.reason,
                              decision.job_limit - db.count_jobs_for_tenant(tenant_id),
    ...
    write_audit_event(tenant_id,
        ...
        "decision", did, {"request_id": rid, "reason": decision.reason})
    ...
    return {"decision_id": did, "result": decision.result.value, "reason": decision.reason,
            "estimated_cost": decision.cost_estimated, "quota_remaining": decision.job_limit,
            "job_id": job_id}
...
async def get_decision(decision_id: str, x_api_key: str = Header(None)):
    ...
    return {"decision_id": rec["id"], "result": rec.get("gate_result", ""),
            "reason": rec.get("gate_reason", ""), "estimated_cost": rec.get("estimated_cost", 0),
            "quota_remaining": rec.get("quota_remaining", 0), "created_at": rec.get("decided_at", "")}


@router.get("")
async def list_decisions(result: str = Query(None), date_from: str = Query(None),
                          date_to: str = Query(None), limit: int = Query(20, le=100),
                          offset: int = Query(0), x_api_key: str = Header(None)):
    tenant_id = _tenant_from_key(x_api_key)
    items, total = db.list_decision_records(tenant_id, result=result, date_from=date_from,
...
async def evaluate_decision(payload: dict, x_api_key: str = Header(None)):
    ...
    pay = payload.get("payload", {})
    request_type = payload.get("request_type", "job_submit")
    pol = evaluate_policies(tenant_id, "decision.evaluate", {"request_type": request_type})
    if pol["result"] != "allowed":
        raise HTTPException(403, pol["reason"])
    decision = gate.evaluate(tenant_id, pay)
    return {"result": decision.result.value, "reason": decision.reason,
            "estimated_cost": decision.cost_estimated, "quota_remaining": decision.job_limit,
            "would_create_job": decision.result.value == "allowed" and request_type == "job_submit"}
