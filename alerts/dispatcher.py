...
class AlertDispatcher:
    ...
    def __init__(self):
        self._telegram_enabled = os.getenv("ALERT_TELEGRAM_ENABLED", "").lower() in (
            "1",
            "true",
            "yes",
        )
        self._discord_enabled = os.getenv("ALERT_DISCORD_ENABLED", "").lower() in (
            "1",
            "true",
            "yes",
        )
        self._email_enabled = os.getenv("ALERT_EMAIL_ENABLED", "").lower() in (
            "1",
            "true",
            "yes",
        )
        ...
        self._email_from = os.getenv(
            "ALERT_EMAIL_FROM", "alerts@roma-execution-bridge.io"
        )
        ...
        if channels:
            ...
            logger.info(
                "AlertDispatcher initialized: no channels enabled (alerts are silent)"
            )

    def send(self, alert: Alert) -> None:
        """Fire-and-forget: dispatch alert to all enabled channels in background thread."""
        if not any(
            [self._telegram_enabled, self._discord_enabled, self._email_enabled]
        ):
            return
    ...
    def channels_status(self) -> dict:
        ...
        return {
            "telegram": {
                ...
                "chat_id": self._tg_chat_id[:8] + "..." if self._tg_chat_id else None,
            },
            "discord": {
                "enabled": self._discord_enabled,
                "webhook_configured": bool(self._discord_webhook_url),
            },
