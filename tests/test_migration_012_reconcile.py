...
@pytest.mark.pg
def test_012_survivor_is_earliest_by_created_at(prod_form):
    """Survivor = самое раннее по (created_at, id), а не по физическому порядку вставки."""
    conn = prod_form
    _apply_012(conn)
    ...
    assert [r[0] for r in cur.fetchall()] == ["earlier"]


@pytest.mark.pg
def test_012_reapply_is_noop(prod_form):
    """Повторный накат 012 — 0 изменений (строки не удаляются, backup не растёт)."""
    conn = prod_form
    _apply_012(conn)
