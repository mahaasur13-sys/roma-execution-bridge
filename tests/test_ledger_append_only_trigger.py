...
def _pg_conn():
    ...
    if not os.environ.get("PG_DSN"):
        pytest.skip("PG_DSN не задан — триггер L1 проверяется только на живом PG")
    ...
    try:
        ...
        pytest.skip("PG недоступен в этом процессе pytest")
    return ctx
...
def _seed_entry(cur):
    ...
    if not fallback:
        pytest.skip("ledger_entries пуста и INSERT недоступен — нечего проверять")
    return fallback[0]
